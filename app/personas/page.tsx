      ...
      <div className="flex-1 pr-80">
        ...
        <header className="sticky top-0 z-30 backdrop-blur-sm border-b px-6 py-4 flex items-center gap-4"
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-semibold" style={{ color: "#111827" }}>User Personas</h1>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            {personas.map((p) => (
              <button key={p.id} onClick={() => setSelected(p.id)}
                ...
                style={{ background: selected === p.id ? "rgba(0,255,150,0.08)" : SURF, borderColor: selected === p.id ? A : BORD }}>
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-xl font-bold mb-3"
                  style={{ background: selected === p.id ? A : "#e5e7eb", color: selected === p.id ? "#111827" : AT }}>
                  {p.name.charAt(0)}
                </div>
                <h3 className="font-semibold text-sm mb-1" style={{ color: "#111827" }}>{p.name}</h3>
                <p className="text-xs" style={{ color: "#6b7280" }}>{p.age_range} · {p.archetype}</p>
              </button>
            ))}
          </div>
          ...
          {active && (
            <div className="grid grid-cols-2 gap-6 animate-fade-in">
              ...
              <div className="rounded-2xl border p-6" style={{ background: SURF, borderColor: BORD }}>
                <h3 className="font-semibold mb-4 flex items-center gap-2" style={{ color: "#111827" }}>
              ...
              <div className="rounded-2xl border p-6" style={{ background: SURF, borderColor: BORD }}>
                <h3 className="font-semibold mb-4 flex items-center gap-2" style={{ color: "#111827" }}>
                  <Zap className="w-4 h-4" style={{ color: AT }} /> Goals
                </h3>
                <ul className="space-y-2">
              ...
              <div className="rounded-2xl border p-6 col-span-2" style={{ background: SURF, borderColor: BORD }}>
                <h3 className="font-semibold mb-4 flex items-center gap-2" style={{ color: "#111827" }}>
                  <Bot className="w-4 h-4" style={{ color: "#7c3aed" }} /> AI Tool Usage & Query Style
                </h3>
                <div className="flex flex-wrap gap-2 mb-4">
                  {active.ai_tools_used?.map((tool) => (
                    <span key={tool} className="px-3 py-1.5 rounded-full text-sm border"
                ...
                <p className="text-sm" style={{ color: "#6b7280" }}>
                  <span className="font-medium" style={{ color: "#374151" }}>Query style: </span>
                  {active.query_style}
                </p>
              </div>
            </div>
